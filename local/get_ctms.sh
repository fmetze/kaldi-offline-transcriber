...
if [ $# -ne 3 ]; then
  echo "Usage: local/get_ctms.sh [--cmd (run.pl|queue.pl...)] <data-dir> <lang-dir|graph-dir> <decode-dir>"
  echo " Options:"
...
data2=`dirname $data`
name=`basename $data2`; # e.g. eval2000
...
if [ $stage -le 0 ]; then
  $cmd ACWT=$min_acwt:$max_acwt $dir/scoring/log/get_ctms.ACWT.log \
